...
[![单文件](https://img.shields.io/badge/架构-单文件-blue?style=flat-square)](./)
[![无依赖](https://img.shields.io/badge/依赖-零框架-2ea44f?style=flat-square)](./)

**单文件 · 零构建 · 无 CDN · 可直接打开**

</div>

> ⚠️ **部署提示**：GitHub Pages 只识别仓库根目录的 `index.html`。当前仓库中的网页文件名为 `YanYan_PersonalOS_motion_visible_single_file.html`，请先将其**重命名为 `index.html`** 并启用 Pages（步骤见下方「部署到 GitHub Pages」），在线地址才会生效。
...
这是我的**个人叙事地图（Personal OS）**：以一个可交互的单页网页，串联组织与人才战略实践、AI-Native 组织研究、学习、摄影与长期训练。

它不是一份静态简历，而是一套**持续生长的个人工作与生活操作系统**：既记录组织现场的真实问题和实践，也保留对技术、学习、创作与身体的长期投入。

### 五个场域

```mermaid
flowchart LR
    CENTER[🧭 ZOE · Personal OS]
    CENTER --> F1[🏢 组织与人才战略实践]
    CENTER --> F2[🤖 AI-Native 组织研究]
    CENTER --> F3[🎓 复旦 MBA · MIT 游学]
    CENTER --> F4[📷 摄影]
    CENTER --> F5[🏃 训练]
```
...
- 🎛️ **交互式场域探索**：点击场域卡片或导航，右侧信息面板随之切换，呈现每个场域此刻的叙事与状态。
...
1. 下载仓库中的 `YanYan_PersonalOS_motion_visible_single_file.html`；
...
> ⚠️ **第一步最关键**：GitHub Pages 只识别根目录的 `index.html`，请务必先把网页文件重命名。
...
1. 将当前单文件网页 `YanYan_PersonalOS_motion_visible_single_file.html` **重命名为 `index.html`**；
2. 在仓库中点击 **Add file → Upload files**（或直接通过 Git 提交）；
3. 上传 `index.html` 与 `README.md`；
...
```text
https://ZoeYan-AIHR.github.io/Yan-PersonalOS/
```
...
```html
<meta property="og:image" content="https://ZoeYan-AIHR.github.io/Yan-PersonalOS/og-cover.png" />
```
...
本仓库用于展示 ZOE 的个人职业实践、研究与生活叙事。

- 页面中的文字、视觉叙事与人物形象，除另有说明外，版权归 ZOE 所有；
